...
__global__ void convoMask(int M, int N, int m, int n, float* image, float* convoMask, float* output){
            int index = blockIdx.x * blockDim.x + threadIdx.x;
            int stride = blockDim.x * gridDim.x;
            int count = M*N;
            int smallCount = m*n;
            for(int i = index; i < count; i += stride){
                ...
                if( (M - i%M) > m && (N - i/M) > n ){
                    ...
                    for(int j = 0; j< smallCount ; j++){
                        output[i] += convoMask[j] * image[i + j%m + M*(j/m)];
                    }
                    if(output[i] < 0)
                        output[i] = 0;
                }
...
__global__ void yuv2bgr(int M, int N, uint16_t* R, uint16_t* G, uint16_t* B)
        {
            ...
            for (int i = index; i < count; i += stride)
            {
				...
				//tu = i/N+0.5f;
				//tv = (i%N)*M+0.5f;
				//y1 = tex2D(yTexRef, tu, tv) - 16;
				//y2 = tex2D(yTexRef, tu+1, tv) - 16;
				//y3 = tex2D(yTexRef, tu+1, tv+1) - 16;
				//y4 = tex2D(yTexRef, tu, tv+1) - 16;
				//u = tex2D(uvTexRef, tu, tv).y;
				//v = tex2D(uvTexRef, tu, tv).x;
				G[((i%N) * 2)*M + (i/N)*2  +1] = (uint16_t)i;//(unsigned char)(Y_SUBSTRACT*y1 - 0.391762*u - 0.812968*v);
				G[((i%N) * 2 + 1)*M + (i/N)*2  +1] = (uint16_t)i;//(unsigned char)(Y_SUBSTRACT*y2 - 0.391762*u - 0.812968*v);
				G[((i%N) * 2)*M + (i/N)*2] = (uint16_t)i;//(unsigned char)(Y_SUBSTRACT*y3 - 0.391762*u - 0.812968*v);
				G[((i%N) * 2 + 1)*M + (i/N)*2  +1] = (uint16_t)i;//(unsigned char)(Y_SUBSTRACT*y4 - 0.391762*u - 0.812968*v);
				B[((i%N) * 2)*M + (i/N)*2  +1] = (uint16_t)i;//(unsigned char)(Y_SUBSTRACT*y1 - 0.391762*u - 0.812968*v);
				B[(i%N + 1)*M + i/N] = (uint16_t)i;//(unsigned char)(Y_SUBSTRACT*y2 - 0.391762*u - 0.812968*v);
				B[(i%N+1)*M + i/N +1] = (uint16_t)i;//(unsigned char)(Y_SUBSTRACT*y3 - 0.391762*u - 0.812968*v);
				B[(i%N)*M + i/N] = (uint16_t)i;//(unsigned char)(Y_SUBSTRACT*y4 - 0.391762*u - 0.812968*v);
				R[((i%N) * 2)*M + (i/N)*2  +1] = (uint16_t)i;//(unsigned char)(Y_SUBSTRACT*y1 - 0.391762*u - 0.812968*v);
				R[(i%N + 1)*M + i/N] = (uint16_t)i;//(unsigned char)(Y_SUBSTRACT*y2 - 0.391762*u - 0.812968*v);
				R[(i%N+1)*M + i/N +1] = (uint16_t)i;//(unsigned char)(Y_SUBSTRACT*y3 - 0.391762*u - 0.812968*v);
				R[(i%N)*M + i/N] = (uint16_t)i;//(unsigned char)(Y_SUBSTRACT*y4 - 0.391762*u - 0.812968*v);
