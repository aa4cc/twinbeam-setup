load('simout_data_18052020_03_frameid.mat')
...
frame_id          = simout.signals(1).values(:,  5 + (2+3+3+3)*N_obj);
%% Ref pos
plot(time, ref_pos)
%% Meas pos
plot(time, meas_pos)
...
f_title = title(sprintf('Time: %5.2f, Frame ID: %d', 0, 0));
...
for I=1:1:numel(time)    
    ...
    f_title.String = sprintf('Time: %5.2f, Frame ID: %d', time(I), frame_id(I));    
