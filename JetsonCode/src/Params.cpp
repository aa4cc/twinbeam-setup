...
void Params::parseJSONIStream(istream& i) {
    ...
    if (j.contains("iters0")) {
        iters0 = j["iters0"].get<int>();
    }
    if (j.contains("iters")) {
        iters = j["iters"].get<int>();
    }
    if (j.contains("mu")) {
        mu = j["mu"].get<double>();
    }
    if (j.contains("rconstr")) {
        rconstr = j["rconstr"].get<std::vector<double>>();
    }
    if (j.contains("iconstr")) {
        iconstr = j["iconstr"].get<std::vector<double>>();
    }
    if(j.contains("cost")) {
      cost = j["cost"].get<bool>();
    }
}
