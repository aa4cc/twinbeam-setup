	...
	while(!appData.appStateIs(AppData::AppState::EXITING)){
		...
		if(input == 's'){
			...
			if(!appData.params.show) {
				...
				appData.params.displayImageType = ImageType::MODULUS;
				if(appData.params.debug) printf("INFO: Switching ON the DISPLAY mode.\n");
			} else {
                switch(appData.params.displayImageType) {
                    case ImageType::MODULUS:
						appData.params.displayImageType = ImageType::PHASE;
						if(appData.params.debug) printf("INFO: Setting the type of the displayed image to MODULUS\n");
                        break;
                    case ImageType::PHASE:
