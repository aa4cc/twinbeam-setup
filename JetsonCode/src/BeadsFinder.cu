...
BeadsFinder::BeadsFinder(uint16_t m, uint16_t n, uint8_t img_thrs, float gaussFiltSigma, cudaStream_t stream, bool dbg): im_width{m}, im_height{n}, img_threshold{img_thrs}, debug{dbg}, stream{stream}
{
...
{ 
    ...
    cudaMemsetAsync(d_pointsCounterPtr, 0, sizeof(uint32_t), stream);

    // Find the local minimums smaller than a given threshold and store their positions to d_positions[]
    getLocalMinima<<<N_BLOCKS, N_THREADS, 0, stream>>>(im_width, im_height, (float*)img_filt.data, d_positions, MAX_NUMBER_BEADS, d_pointsCounterPtr, img_threshold);

    // Copy back the value of the counter and the array to the CPU memory
    {
        std::lock_guard<std::mutex> l(_mtx);
        cudaMemcpyAsync(&pointsCounter, d_pointsCounterPtr, sizeof(uint32_t), cudaMemcpyDeviceToHost, stream);
        cudaMemcpyAsync(&positions, d_positions, 2*pointsCounter*sizeof(uint16_t), cudaMemcpyDeviceToHost, stream);
    }
