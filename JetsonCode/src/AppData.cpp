...
AppData::AppData() {
	...
	phase_is_initialized = true;

	cudaStreamCreate(&streamBack);
	cudaStreamCreate(&streamPhase);
	cudaStreamCreate(&streamNetwork);
	...
	ImageData<uint8_t> G, R, G_bckp, R_bckp, modulus, phase, raw_phase;
...
bool AppData::waitTillAppIsInitialized(useconds_t useconds) {
	// Wait till the app enters the appState or AppState::EXITING
	while((!camera_is_initialized || !display_is_initialized || !imgproc_is_initialized || !phase_is_initialized) && appState != AppState::EXITING) { usleep(useconds); }
